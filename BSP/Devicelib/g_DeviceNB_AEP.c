...
/*******************************************************************************
* 函数名    : NB_ConfigEndPoint
* 描述	  	: 设置EndPoint，默认使用IMEI认证，先获取IMEI，然后配置参数
* 输入参数  : resource---目标字符串，dest---目标数组， sourceLen----字符串长度
* 返回参数  : 无
*******************************************************************************/
void NB_ConfigEndPoint(void)
{
	// +CGSN:865057047986793
	char* a;
	uint8_t i = 0;
	uint8_t cmd_endpoint[50] = "AT+QCFG=\"LWM2M/EndpointName\",\"865057047986793\"\r\n";
//	uint8_t imei[] = "865057047986793";
	Clear_Buffer((unsigned char *)aRxBuff,&aRxNum);
	User_Printf((char *)"AT+CGSN=1\r\n");
	OSTimeDly(100);
	a = strstr(aRxBuff,"+CGSN:");//用于判断字符串str2是否是str1的子串。如果是，则该函数返回str2在str1中首次出现的地址；否则，返回NULL。
	if(a!=NULL)
	{
		for(i = 0; i < 15; i++){
			cmd_endpoint[30+i] = a[6+i];
		}
		// cmd_endpoint[30+i] = '\0';
		NB_Config(cmd_endpoint,5,5);
		// g_Printf_dbg(cmd_endpoint);
		// OSBsp.Device.Usart0.WriteNData(cmd_endpoint,48);
		// OSTimeDly(100);
	}
...
char g_Device_NB_Init(void)
{
	...
	if(AppDataPointer->TransMethodData.NBStatus == NB_Boot)
	{
		...
		while (ii < 60)
		{
			...
			OSTimeDly(1000);
			if(Hal_CheckString(aRxBuff,"+QLWEVTIND:3"))
			{
...
//	static uint16_t complement=0;
	...
	if(a!=NULL)
	{
		...
		/*CTwing发送数组*/
		AppDataPointer->TransMethodData.CSQ = dataTemp;
		...
		// /**********************************************/
		// dataTemp=0-dataTemp;
		// AppDataPointer->TransMethodData.CSQ = (float)dataTemp/10;
