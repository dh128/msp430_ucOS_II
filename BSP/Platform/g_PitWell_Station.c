...
const uint8_t ScadaLV[CMDLength] 	= {0x02, 0x03, 0x00, 0x0B, 0x00, 0x01, 0xF5, 0xFB};	 //投入式液位
	...
	if (AppDataPointer->TerminalInfoData.SensorStatus != 0)
	{
		...
		for (scadaIndex = 1; scadaIndex <= SensorNum; scadaIndex++) //SensorNum = 12
		{
			...
			if(sensorExistStatus == 1)
			{
				...
				case 2:
					sensorSN = 2;
					OSBsp.Device.Usart3.WriteNData((uint8_t *)ScadaLV, CMDLength);
					break;
				default:
					break;
				}
...
char *MakeJsonBodyData(DataStruct *DataPointer)
{
	...
	/* 超声波液位超过0.6m，则认为没有水，投入式传感器检测数据为大气压值 */
	if(App.Data.PitWellData.Ultrasonic > 0.6){
		App.Data.PitWellData.AirPress = App.Data.PitWellData.Input;
	}
	/* 实际液位计算 */
	App.Data.PitWellData.Input -= App.Data.PitWellData.AirPress;	//投入式液位需要减去大气压值
	if(App.Data.PitWellData.Input < 0){
		App.Data.PitWellData.Input = 0;
	}
	if(App.Data.PitWellData.Ultrasonic > 0.39){
		App.Data.PitWellData.Real = App.Data.PitWellData.Height - App.Data.PitWellData.Ultrasonic;
		...
		if(App.Data.PitWellData.Input >= 0.02){
			...
			App.Data.PitWellData.Real = 0;		/* 0--故障 */
		}
	}
	/* 根据液位深度调整上报周期，周期不存储，每次开机默认最长周期，然后根据实际液位调整 */
	if(App.Data.PitWellData.Real >= App.Data.PitWellData.DangerHeight){
		App.Data.TerminalInfoData.SendPeriod = 5;	/* 危险周期 */
...
#endif //(PRODUCT_TYPE == IntegratedPitWell)
